...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that Funnyarch uses to lower LLVM code into
// a selection DAG.
//
//===----------------------------------------------------------------------===//
...
const char *FunnyarchTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case FunnyarchISD::Ret:
    return "FunnyarchISD::Ret";
  default:
    return NULL;
  }
}

void FunnyarchTargetLowering::ReplaceNodeResults(
...
static const MCPhysReg GPRArgRegs[] = {
    ...
    Funnyarch::R4, Funnyarch::R5, Funnyarch::R7, Funnyarch::R7};
...
bool FunnyarchTargetLowering::CanLowerReturn(
...
SDValue
FunnyarchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                   SmallVectorImpl<SDValue> &InVals) const {
  llvm_unreachable("Cannot lower call");
}

SDValue FunnyarchTargetLowering::LowerReturn(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
...
SDValue FunnyarchTargetLowering::LowerGlobalAddress(SDValue Op,
                                                    SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported global address");
}

SDValue FunnyarchTargetLowering::LowerBlockAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported block address");
}

SDValue FunnyarchTargetLowering::LowerConstantPool(SDValue Op,
                                                   SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported constant pool");
}

SDValue FunnyarchTargetLowering::LowerRETURNADDR(SDValue Op,
                                                 SelectionDAG &DAG) const {
  return SDValue();
}

SDValue FunnyarchTargetLowering::LowerShlParts(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue FunnyarchTargetLowering::LowerShrParts(SDValue Op, SelectionDAG &DAG,
                                               bool arith) const {
  assert(Op.getNumOperands() == 3 && "Not a long shift");
...
SDValue FunnyarchTargetLowering::LowerOperation(SDValue Op,
                                                SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
