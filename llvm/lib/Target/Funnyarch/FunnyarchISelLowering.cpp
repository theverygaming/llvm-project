//===-- FunnyarchISelLowering.cpp - Funnyarch DAG Lowering Implementation -----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that Funnyarch uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "funnyarch-isellower"

#include "FunnyarchGenCallingConv.inc"

FunnyarchTargetLowering::FunnyarchTargetLowering(const TargetMachine &TM,
                                         const FunnyarchSubtarget &STI)
    : TargetLowering(TM), Subtarget(STI)
{
  // Set up the register classes
  addRegisterClass(MVT::i32, &Funnyarch::GPRRegClass);
  ...
  setStackPointerRegisterToSaveRestore(Funnyarch::RSP);
...
const char *FunnyarchTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case FunnyarchISD::Ret: return "FunnyarchISD::Ret";
  default:            return NULL;
  }
}

void FunnyarchTargetLowering::ReplaceNodeResults(SDNode *N,
...
static const MCPhysReg GPRArgRegs[] = {
  Funnyarch::R0, Funnyarch::R1, Funnyarch::R2, Funnyarch::R3, Funnyarch::R4, Funnyarch::R5, Funnyarch::R7, Funnyarch::R7
};

/// LowerFormalArguments - transform physical registers into virtual registers
/// and generate load operations for arguments places on the stack.
SDValue FunnyarchTargetLowering::LowerFormalArguments(
                                    SDValue Chain,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, Funnyarch_CCallingConv);
  ...
  unsigned ArgRegBegin = Funnyarch::R4;
...
bool FunnyarchTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                MachineFunction &MF, bool isVarArg,
                                const SmallVectorImpl<ISD::OutputArg> &Outs,
                                LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
  return CCInfo.CheckReturn(Outs, Funnyarch_CRetConv);
}

SDValue FunnyarchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                       SmallVectorImpl<SDValue> &InVals) const {
  llvm_unreachable("Cannot lower call");
}

SDValue
FunnyarchTargetLowering::LowerReturn(SDValue Chain,
  ...
  CCInfo.AnalyzeReturn(Outs, Funnyarch_CRetConv);
  ...
  return DAG.getNode(FunnyarchISD::Ret, dl, MVT::Other, RetOps);
}
...
SDValue
FunnyarchTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported global address");
}

SDValue
FunnyarchTargetLowering::LowerBlockAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported block address");
}

SDValue
FunnyarchTargetLowering::LowerConstantPool(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported constant pool");
}

SDValue
FunnyarchTargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
  return SDValue();
}

SDValue
FunnyarchTargetLowering::LowerShlParts(SDValue Op, SelectionDAG &DAG) const {
...
SDValue
FunnyarchTargetLowering::LowerShrParts(SDValue Op, SelectionDAG &DAG,
                                   bool arith) const {
  assert(Op.getNumOperands() == 3 && "Not a long shift");
...
SDValue
FunnyarchTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
