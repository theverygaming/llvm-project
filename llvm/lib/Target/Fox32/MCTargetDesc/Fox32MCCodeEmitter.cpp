//===-- Fox32MCCodeEmitter.cpp - Convert Fox32 code emitter -----*- C++ -*-===//
//
...
///
/// \file
/// This file contains defintions for Fox32 code emitter.
///
//===----------------------------------------------------------------------===//

#include "MCTargetDesc/Fox32MCCodeEmitter.h"
...
#define DEBUG_TYPE "fox32-mccodeemitter"

namespace {
class Fox32MCCodeEmitter : public MCCodeEmitter {
  Fox32MCCodeEmitter(const Fox32MCCodeEmitter &) = delete;
  void operator=(const Fox32MCCodeEmitter &) = delete;
  const MCInstrInfo &MCII;
  MCContext &Ctx;

  void getMachineOpValue(const MCInst &MI, const MCOperand &MO, APInt &Value, 
                             SmallVectorImpl<MCFixup> &Fixups,
                             const MCSubtargetInfo &STI) const;

  void getImmOpValue(const MCInst &MI, unsigned OpNo, APInt &Value,
                         SmallVectorImpl<MCFixup> &Fixups,
                         const MCSubtargetInfo &STI) const;
...
public:
  Fox32MCCodeEmitter(const MCInstrInfo &mcii, MCContext &ctx)
      : MCII(mcii), Ctx(ctx) {}

  ~Fox32MCCodeEmitter() override {}
...
#include "Fox32GenMCCodeEmitter.inc"

void
Fox32MCCodeEmitter::getMachineOpValue(const MCInst &MI, const MCOperand &MO, APInt &Value, 
                                      SmallVectorImpl<MCFixup> &Fixups,
                                      const MCSubtargetInfo &STI) const {
  Value = 0;
  /*
...
void Fox32MCCodeEmitter::getImmOpValue(const MCInst &MI, unsigned OpNo, APInt &Value,
                                           SmallVectorImpl<MCFixup> &Fixups,
                                           const MCSubtargetInfo &STI) const {
...
void Fox32MCCodeEmitter::encodeInstruction(const MCInst &MI, raw_ostream &OS,
                                           SmallVectorImpl<MCFixup> &Fixups,
                                           const MCSubtargetInfo &STI) const {
  ...
  int64_t InstBytes = MCII.get(MI.getOpcode()).getSize();
  printf("instBytes: %d\n", InstBytes);
  for (uint64_t Word : Data) {
    for (int i = 0; i < 8 && InstBytes > 0; ++i, InstBytes -= 1) {
      OS.write(static_cast<uint8_t>(Word));
      Word >>= 8;
      printf("W: 0x%x\n", Word & 0xFF);
    }
  }
...
MCCodeEmitter *llvm::createFox32MCCodeEmitter(const MCInstrInfo &MCII,
                                              MCContext &Ctx) {
  return new Fox32MCCodeEmitter(MCII, Ctx);
}
